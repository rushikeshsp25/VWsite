...
@login_required
@user_passes_test(lambda u: u.has_perm('home.view_onlinecampaign'), login_url='/permissionerror/',redirect_field_name=None)
        ...
        if form.is_valid():
            ...
            for entry in entries:
                if entry!='':
                    ...
                    cert.conducted_on = conducted_on
...
@login_required
@user_passes_test(lambda u: u.is_superuser,login_url='/permissionerror/')
def certification_entries(request):
    student_certificates = StudentCertification.objects.all()
    return render(request, 'home/certification/certification_entries.html',{'student_certificates':student_certificates})
